-- ============================================================================
-- KRIEL — Supabase setup (with 3 new features)
-- Schema + RLS policies + seed data.
--
-- Run this ONCE in the Supabase dashboard:  SQL Editor > New query > paste > Run
-- This is safe to re-run on existing installations (uses IF NOT EXISTS / ADD COLUMN IF NOT EXISTS).
-- ============================================================================
...
-- For EXISTING installations: these no-op when the columns already exist.
...
-- ============================================================================
-- NEW COLUMNS for the 3 new features (safe to run on existing installations):
--
-- Feature 1: price_per_char — base price per character for custom text.
--   Each variant in the variants[] JSONB array also gets a pricePerChar field.
--   This column is the product-level fallback when no variant-specific value is set.
--
-- Feature 3: color_images — maps colour names to arrays of image URLs.
--   { "Black": ["url1","url2"], "Gold": ["url3"] }
-- ============================================================================

alter table products add column if not exists price_per_char numeric(10, 4) not null default 0;
...
-- --------------------------------- SEED -------------------------------------
-- Skipped automatically if categories exist.
...
-- ============================================================================
-- MIGRATION for existing Kriel installations:
-- Run ONLY if you already have the old Kriel DB running and want to add the
-- 3 new features without recreating everything. These statements are safe to
-- run on a fresh DB too (the ALTER TABLE ... ADD COLUMN IF NOT EXISTS is idempotent).
-- ============================================================================

-- Feature 1: price per character for custom text
alter table products add column if not exists price_per_char numeric(10, 4) not null default 0;

-- Feature 3: colour-specific images
alter table products add column if not exists color_images jsonb not null default '{}'::jsonb;

-- Note: Feature 2 (nga €X pricing in sets) is purely frontend — no DB change needed.
-- Note: Feature 1 per-variant pricePerChar is stored inside the variants JSONB array,
--       so no separate column is needed for per-variant text pricing.
