...
export async function fetchProducts(options?: {
  ...
  let query = getSupabase()
    .from("products")
    .select(
      "id, category_id, name, code, price, dims, material, description, image, images, sizes, colors, variants, sale_pct, sort_order, featured, featured_order, customizable, active, set_name"
    )
    .order("sort_order", { ascending: true })
    .order("id", { ascending: true });
...
export type ProductInput = {
  ...
  sortOrder?: number;
  /** Optional set name for grouping complementary products. */
  setName?: string;
};
...
export async function updateProduct(
  ...
  if (typeof patch.setName === "string") {
    updates.set_name = patch.setName.trim() || null;
  }
