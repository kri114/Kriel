...
import { Home, Grid3X3, BookOpen, ShoppingBag, Phone } from "lucide-react";
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 inset-x-0 z-[60] px-4 pb-[max(0.85rem,env(safe-area-inset-bottom))]">
      <div className="mx-auto max-w-md rounded-full border border-bronze/25 bg-ink/88 backdrop-blur-xl shadow-[0_-8px_40px_rgba(0,0,0,0.5)] px-2 py-2 flex justify-between">
        ...
        <button onClick={openCart} className="relative flex flex-col items-center gap-0.5 rounded-full px-3.5 py-1.5 text-ivory-2/65">
          <ShoppingBag size={17} strokeWidth={1.7} />
          {count > 0 && (
            <span className="absolute top-0.5 right-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-bronze px-1 text-[8.5px] font-bold text-ink">
              {count}
            </span>
          )}
          <span className="text-[9px] font-bold tracking-[0.12em] uppercase">Shporta</span>
        </button>
      </div>
    </nav>
  );
