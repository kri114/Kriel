...
import { getProductCode } from "@/lib/constants";
...
export function CartProvider({ children }: { children: ReactNode }) {
    ...
    setItems((prev) => {
      ...
      return [
        ...prev,
        {
          ...
          name: product.name,
          code: opts?.code ?? getProductCode(product, color),
          price: opts?.price ?? product.price,
          textPricePerCharacter: opts?.textPricePerCharacter ?? 0,
  ...
  // A custom-text price replaces the normal product/variant price.
  // With no custom text, the normal product/variant price is used.
  const total = useMemo(() => items.reduce((sum, it) => {
    const hasCustomText = Boolean(it.customText?.trim());
    const unitPrice = hasCustomText
      ? (it.customText!.length * (it.textPricePerCharacter || 0))
      : it.price;
    return sum + it.qty * unitPrice;
  }, 0), [items]);
