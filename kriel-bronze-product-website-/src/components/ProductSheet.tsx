...
export default function ProductSheet({
  ...
  const colorSpecificPhotos = (color && p.colorImages?.[color]) || [];
  const photos = colorSpecificPhotos.length > 0 
    ? colorSpecificPhotos 
  ...
  const textPricePerCharacter = currentVariant?.pricePerCharacter ?? 0;
  const textCost = (customText.length) * textPricePerCharacter;
  
  const onSale = (p.salePct ?? 0) > 0;
  const currentPrice = effectivePrice(basePrice, p.salePct) + textCost;
  ...
  const waText = `Përshëndetje KRIEL! Jam i/e interesuar për produktin "${p.name}"${currentCode ? ` (${currentCode})` : ""}, çmimi ${fmtEUR(currentPrice)}.${
    size ? ` Madhësia: "${size}".` : ""
  }${color ? ` Ngjyra: "${color}".` : ""}${
  ...
  const handleAdd = () => {
    ...
    addItem(p, { 
      ...
      size, 
      color, 
      price: currentPrice - textCost, 
      textPricePerCharacter,
      code: currentCode 
    });
    setAdded(true);
            ...
            <div className="mt-3.5 flex items-end justify-between rounded-2xl border border-line bg-ink-3/50 px-4 py-3.5">
              <div>
                ...
                <p className="mt-0.5 flex items-baseline gap-2.5">
                  ...
                  {onSale && (
                    <span className="text-[15px] font-semibold line-through text-ivory-2/45">{fmtEUR(basePrice + (textCost / (1 - (p.salePct || 0)/100)))}</span>
                  )}
                </p>
                {textCost > 0 && (
                  <p className="mt-1 text-[10px] font-medium text-emerald-500/90">
                    Përfshin {fmtEUR(textCost)} për {customText.length} gërma ({fmtEUR(textPricePerCharacter)}/gërmë)
                  </p>
                )}
                {onSale && (
                  ...
                  <div className="border-t border-bronze/20 px-4 py-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {setProducts.map((sp) => {
                        ...
                        const prices = [sp.price, ...sp.variants.map(v => v.price)].filter(p => p > 0);
                          ...
                          >
                            ...
                            <div className="mt-2 px-1">
                              ...
                              <p className="mt-1 text-[11px] font-bold text-bronze">
                                {hasMultiplePrices ? "nga " : ""}
                                {fmtEUR(effectivePrice(minPrice, sp.salePct))}
                              </p>
                            </div>
                          </button>
                        );
