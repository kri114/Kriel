...
import { fmtEUR, effectivePrice, PHONE_TEL, waLink, getProductCode, getProductSets } from "@/lib/constants";
...
/**
 * Feature 2: Get the minimum price for a product.
 * Returns the smallest price across all variants, or the base price if no variants.
 */
function getMinPrice(p: Product): number {
  if (p.variants.length === 0) return p.price;
  const variantPrices = p.variants.map((v) => v.price);
  return Math.min(...variantPrices);
}

/**
 * Feature 2: Check whether a product has multiple distinct prices.
 */
function hasMultiplePrices(p: Product): boolean {
...
export default function ProductSheet({
  ...
  const productSets = getProductSets(p);
  const setProducts = productSets.length > 0
    ? allProducts.filter(
        (x) => x.id !== p.id && x.active && getProductSets(x).some((s) => productSets.includes(s))
      )
    : [];

  // Feature 3: determine photos based on selected colour
  const colorPhotos: string[] = color && p.colorImages && p.colorImages[color] && p.colorImages[color].length > 0
    ? p.colorImages[color]
    : [];
  const defaultPhotos = p.images.length ? p.images : p.image ? [p.image] : [];
  // Use colour-specific photos if available, otherwise fall back to default
  const photos = colorPhotos.length > 0 ? colorPhotos : defaultPhotos;
  ...
  const currentVariant = hasVariants
    ...
    : undefined;
  const currentCode = getProductCode(p, color, currentVariant);
  ...
  // Feature 1: calculate price per character for current variant
  // Priority: variant's pricePerChar → product's base pricePerChar → 0
  const pricePerChar = currentVariant?.pricePerChar && currentVariant.pricePerChar > 0
    ? currentVariant.pricePerChar
    : (p.pricePerChar && p.pricePerChar > 0 ? p.pricePerChar : 0);

  const charCount = customText.length; // All characters including special chars and spaces
  const textCost = pricePerChar > 0 && charCount > 0 ? pricePerChar * charCount : 0;
  const totalPrice = currentPrice + textCost;

  const pickColor = (c: string) => {
    setColor(c);
    setSize(sizesFor(c)[0] ?? "");
    setPhotoIdx(0); // reset photo index when colour changes (Feature 3)
  };

  const waText = `Përshëndetje KRIEL! Jam i/e interesuar për produktin "${p.name}"${currentCode ? ` (${currentCode})` : ""}, çmimi ${fmtEUR(totalPrice)}.${
    size ? ` Madhësia: "${size}".` : ""
  }${color ? ` Ngjyra: "${color}".` : ""}${
  ...
  const handleAdd = () => {
    if (!canAdd) return;
    addItem(p, { qty, customText: customText.trim(), size, color, price: totalPrice, code: currentCode });
    setAdded(true);
      ...
      >
        ...
        <div className="overflow-y-auto no-scrollbar" style={{ maxHeight: "calc(92svh - 20px)" }}>
          ...
          >
            ...
            {/* Feature 3: show colour badge when using colour-specific photos */}
            {colorPhotos.length > 0 && color && (
              <span className="absolute top-2.5 left-2.5 rounded-full bg-ink/70 backdrop-blur px-2.5 py-1 text-[10px] font-semibold text-ivory-2">
                {color}
              </span>
            )}
          </div>
          ...
          <div className="px-5 pt-5 pb-7">
            ...
            {currentCode ? (
              <p className="mt-1.5 text-[11.5px] font-medium tracking-[0.22em] uppercase text-ivory-2/50">{currentCode}</p>
            ) : null}
            ...
            <div className="mt-3.5 flex items-end justify-between rounded-2xl border border-line bg-ink-3/50 px-4 py-3.5">
              <div>
                ...
                <p className="mt-0.5 flex items-baseline gap-2.5">
                  <span className="font-display text-[32px] font-bold leading-none text-bronze-grad">{fmtEUR(totalPrice)}</span>
                  {onSale && (
                ...
                {/* Feature 1: Show text cost breakdown */}
                {textCost > 0 && (
                  <div className="mt-2 space-y-0.5">
                    <p className="text-[10.5px] text-ivory-2/60">
                      Çmimi bazë: <span className="text-ivory-2">{fmtEUR(currentPrice)}</span>
                    </p>
                    <p className="text-[10.5px] text-bronze/80">
                      Teksti ({charCount} karaktere × {fmtEUR(pricePerChar)}): <span className="text-bronze font-semibold">+{fmtEUR(textCost)}</span>
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2.5">
              {currentCode && <Spec icon={Tag} k="Kodi i katalogut" v={currentCode.replace("COD. ", "")} />}
              {p.dims && <Spec icon={Ruler} k="Përmasat" v={p.dims} />}
              {p.material && <Spec icon={Layers} k="Materiali" v={p.material} />}
              {pricePerChar > 0 && (
                <Spec icon={PenLine} k="Çmimi/karakter" v={fmtEUR(pricePerChar)} />
              )}
            </div>
            ...
            {/* Feature 2: Set products with "nga €X" for multi-price products */}
              ...
              <div className="mt-5 overflow-hidden rounded-2xl border border-bronze/30 bg-bronze/5">
                ...
                >
                  <span className="text-[13px] font-semibold text-ivory">
                    Pjesë e {productSets.length === 1 ? "setit" : "seteve"}{" "}
                    <span className="text-bronze">{productSets.join(", ")}</span>
                  </span>
                  <ChevronDown
                    size={16}
                ...
                {showSet && (
                  <div className="border-t border-bronze/20 px-4 py-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {setProducts.map((sp) => {
                        const spCode = getProductCode(sp);
                        // Feature 2: show "nga €X" if product has multiple prices
                        const spMultiPrice = hasMultiplePrices(sp);
                        const spMinPrice = getMinPrice(sp);
                        const spDisplayPrice = effectivePrice(spMinPrice, sp.salePct);
                        return (
                          <button
                            key={sp.id}
                            onClick={() => {
                          ...
                          >
                            ...
                            <div className="mt-2 px-1">
                              ...
                              {spCode && (
                                <p className="text-[9px] font-medium tracking-wider uppercase text-ivory-2/40">{spCode}</p>
                              )}
                              <p className="mt-1 text-[11px] font-bold text-bronze">
                                {spMultiPrice ? (
                                  <span>
                                    nga {fmtEUR(spDisplayPrice)}
                                  </span>
                                ) : (
                                  fmtEUR(spDisplayPrice)
                                )}
                              </p>
                            </div>
                          </button>
              ...
              <div className="mt-5">
                ...
                <div className="mt-2.5 flex flex-wrap gap-2">
                  {colorsList.map((opt) => (
                    ...
                    >
                      {opt}
                      {/* Feature 3: indicator for colour with specific images */}
                      {p.colorImages && p.colorImages[opt] && p.colorImages[opt].length > 0 && (
                        <span className="ml-1.5 inline-block w-1.5 h-1.5 rounded-full bg-bronze/70 align-middle" title={`${p.colorImages[opt].length} foto`} />
                      )}
                    </button>
                  ))}
                </div>
            ...
            {p.customizable && (
              <div className="mt-5 rounded-2xl border border-bronze/35 bg-bronze/[0.06] p-4">
                ...
                />
                {/* Feature 1: Live character count and cost preview */}
                {customText.length > 0 && pricePerChar > 0 && (
                  <div className="mt-2 rounded-xl border border-bronze/25 bg-ink-3/40 px-3 py-2">
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="text-ivory-2/60">
                        {customText.length} karaktere × {fmtEUR(pricePerChar)}/karakter
                      </span>
                ...
                {customText.length > 0 && pricePerChar === 0 && (
                  <p className="mt-1.5 text-[10.5px] text-ivory-2/45">
                    {customText.length} karaktere
                  </p>
                )}
              </div>
            )}
