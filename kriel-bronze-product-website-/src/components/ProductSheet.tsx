...
export default function ProductSheet({
  ...
  const textPrice = customText.trim().length * textPricePerCharacter;
  
  const onSale = (p.salePct ?? 0) > 0;
  // When custom text exists, its character price is the complete price.
  // The normal product/variant price is used only when there is no custom text.
  const currentPrice = hasCustomText
    ? textPrice
    : effectivePrice(basePrice, p.salePct);
  ...
  const handleAdd = () => {
    ...
    addItem(p, { 
      ...
      size, 
      color, 
      price: effectivePrice(basePrice, p.salePct),
