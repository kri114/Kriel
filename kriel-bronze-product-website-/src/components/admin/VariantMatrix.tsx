...
export default function VariantMatrix({
  ...
  const setCell = (color: string, size: string, field: 'price' | 'pricePerCharacter', raw: string) => {
    ...
    // When adding only a character price to a new variant, preserve the
    // product's existing base price instead of creating a zero-price variant.
    // This keeps the normal product price completely independent from the
    // per-character price.
    const existing = value.find((v) => v.color === color && v.size === size);
    const current: ProductVariant = existing
      ? { ...existing }
      : { color, size, price: basePrice, pricePerCharacter: 0 };
