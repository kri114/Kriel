...
import { getSupabase, getSupabasePublicUrl } from "@/lib/supabase";
...
export default function OrdersTab({ orders, reload }: { orders: Order[]; reload: () => Promise<void> }) {
  ...
  const testEmail = async () => {
    ...
    try {
      ...
      if (error) {
        setTestMsg({
          ok: false,
          text: `Funksioni nuk u thirr (${error.message}). Paneli po thërret: ${getSupabasePublicUrl()}/functions/v1/send-order-email — ky DUHET të jetë i njëjti projekt ku e deploy-uat funksionin (krahasoni ref-in e URL-së me atë të dashboard-it tuaj Supabase). Kontrolloni gjithashtu: 1) funksioni është deploy-uar me kodin e ri në PO KËTË projekt, 2) "Enforce JWT Verification" është OFF, 3) faqja është ribuild-uar në Render.`,
        });
        return;
      }
  ...
  return (
    <div className="space-y-4">
      ...
      {orders.length === 0 && <p className="text-slate-700 text-sm">Ende nuk ka porosi.</p>}
      {orders.map((o) => (
        <div key={o.id} className="rounded-2xl border border-slate-200 bg-white p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="text-[12px] text-slate-600 mt-0.5">{new Date(o.createdAt).toLocaleString("sq-AL")}</p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => remove(o.id)} className="text-slate-600 hover:text-red-500">
                <Trash2 size={15} />
              </button>
            </div>
          </div>
          ...
          <div className="mt-3 grid sm:grid-cols-3 gap-3 text-[13px] text-slate-700">
            ...
            <p className="flex items-center gap-1.5"><Phone size={13} className="text-slate-600" /> {o.phone}</p>
            <p className="flex items-center gap-1.5"><MapPin size={13} className="text-slate-600" /> {o.address}</p>
          </div>
          {o.notes && <p className="mt-2 text-[12.5px] text-slate-700 italic">Shënim: {o.notes}</p>}
          ...
          <div className="mt-3 rounded-xl bg-slate-50 border border-slate-200 divide-y divide-slate-200">
            {o.items.map((it, i) => (
              <div key={i} className="flex items-center justify-between gap-3 px-3.5 py-2.5">
                <div className="min-w-0">
                  <p className="text-[13px] font-medium text-slate-800 truncate">{it.name} {it.code && <span className="text-slate-600 font-normal">({it.code})</span>}</p>
                  {it.size && <p className="text-[11.5px] text-amber-700">Madhësia: &ldquo;{it.size}&rdquo;</p>}
                  {it.color && <p className="text-[11.5px] text-amber-700">Ngjyra: &ldquo;{it.color}&rdquo;</p>}
