...
function pruneVariants(d: Draft): ProductVariant[] {
  ...
  // Keep a variant when it has either a normal price OR a per-character
  // price. A character-only entry must never erase/change the normal price.
  return d.variants.filter(
    (v) =>
      colors.includes(v.color) &&
      sizes.includes(v.size) &&
      (Number(v.price) > 0 || Number(v.pricePerCharacter ?? 0) > 0)
  );
}
        ...
        <>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <ColorImagesField
              ...
              value={newDraft.colorImages}
              onChange={(v) => setNewDraft({ ...newDraft, colorImages: v })}
            />
                  ...
                  {!isEditing ? (
                    ...
                    <div>
                      ...
                      <ColorImagesField
                        ...
                        value={draft?.colorImages ?? p.colorImages ?? {}}
                        onChange={(v) =>
                          setDrafts((d) => ({
                            ...d,
                            [p.id]: { ...(d[p.id] ?? toDraft(p)), colorImages: v },
                          }))
                        }
                      />
