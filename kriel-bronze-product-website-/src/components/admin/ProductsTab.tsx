...
import ColorImagesField from "./ColorImagesField";
import {
...
function toDraft(p: Product): Draft {
  ...
  return {
    ...
    colorImages: p.colorImages ? { ...p.colorImages } : {},
    price: p.price,
    dims: p.dims,
    material: p.material,
                ...
                >
                  {!isEditing ? (
                    <>
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {(() => {
                            const sets = getProductSets(p);
                            const colorCodeEntries = Object.entries(p.colorCodes ?? {}).filter(([, c]) => Boolean(c));
                            const colorImgEntries = Object.entries(p.colorImages ?? {}).filter(([, imgs]) => imgs.length > 0);
                            const pricePerChar = p.pricePerChar ?? 0;
                            if (sets.length === 0 && colorCodeEntries.length === 0 && colorImgEntries.length === 0 && pricePerChar <= 0) return null;
                            return (
                    ...
                    <div>
                      ...
                      <ProductDraftForm
                        draft={draft ?? toDraft(p)}
                        setDraft={(updated) => setDrafts((d) => ({ ...d, [p.id]: updated }))}
                        categories={categories}
                      />
...
/** Shared form for new product draft and existing product edit draft */
    ...
    <>
      <div className="grid sm:grid-cols-2 gap-4">
        <Labeled label="Emri">
          <input
            ...
            placeholder={isNew ? "Emri i produktit të ri..." : undefined}
          />
        </Labeled>
        ...
        <Labeled label="Kategoria">
          <select
            ...
            onChange={(e) => upd({ categoryId: e.target.value ? Number(e.target.value) : null })}
            className="w-full rounded-lg border border-slate-300 px-3 py-2 text-[14px]"
          >
        ...
        <Labeled label="Përmasat">
          <input
            value={draft.dims}
            onChange={(e) => upd({ dims: e.target.value })}
            className="w-full rounded-lg border border-slate-300 px-3 py-2 text-[14px]"
            placeholder="p.sh. Lartësia 40 cm"
          />
        </Labeled>
        <Labeled label="Materiali">
          <input
            ...
            className="w-full rounded-lg border border-slate-300 px-3 py-2 text-[14px]"
            placeholder="p.sh. Bronz i punuar dorë"
          />
        </Labeled>
      </div>
      ...
      <div className="mt-4">
        <OptionsField
          ...
          value={draft.setNames}
          onChange={(v) => upd({ setNames: v, setName: v[0] ?? "" })}
        />
      </div>
      ...
      <div className="mt-4">
        <ImagesField
          value={draft.images}
          onChange={(v) => upd({ images: v, image: v[0] ?? "" })}
        />
      </div>
      ...
      <div className="mt-4 grid sm:grid-cols-2 gap-4">
        ...
        <OptionsField
          ...
          value={draft.colors}
          onChange={(v) => upd({ colors: v })}
        />
      </div>

      <ColorCodesField
        colors={draft.colors}
        colorCodes={draft.colorCodes}
        baseCode={draft.code}
        onChange={(codes) => upd({ colorCodes: codes })}
      />

      {/* Feature 3: Color-specific images */}
      {draft.colors.length > 0 && (
        <ColorImagesField
          colors={draft.colors}
          colorImages={draft.colorImages}
          onChange={(ci) => upd({ colorImages: ci })}
        />
      )}
      ...
      {(draft.colors.length > 0 || draft.sizes.length > 0) && (
        <div className="mt-4">
          <VariantMatrix
            ...
            onChange={(v) => upd({ variants: v })}
            isCustomizable={draft.customizable}
          />
        </div>
      )}

      {/* Customizable checkbox + base pricePerChar */}
      <div className="mt-4 space-y-3">
        ...
        {/* Feature 1: Base price per character (when no variants or as fallback) */}
        {draft.customizable && (
          <div className="ml-6 rounded-xl border border-emerald-200 bg-emerald-50 p-3.5">
            <span className="block text-[11px] font-semibold uppercase tracking-wide text-emerald-800 mb-1">
              Çmimi për karakter (tekst i personalizuar)
            </span>
            <p className="text-[10.5px] text-emerald-700 mb-2">
              Çmimi bazë për çdo karakter/shkronjë të tekstit të personalizuar.
              Nëse produkti ka variante, çmimi mund të vendoset veçmas për çdo variant (shih matricën lart).
              {draft.variants.length > 0
                ? " Ky çmim bazë do të aplikohet vetëm kur varianti nuk ka çmim/karakter të caktuar."
                : " Ky çmim do të aplikohet për të gjitha variantet."}
            </p>
            <div className="flex items-center gap-2">
              <span className="text-[13px] font-semibold text-emerald-700">€</span>
              <input
                type="number"
                min="0"
                step="0.01"
                value={draft.pricePerChar > 0 ? draft.pricePerChar : ""}
                onChange={(e) => {
                  const v = Number(e.target.value);
                  upd({ pricePerChar: Number.isFinite(v) && v > 0 ? v : 0 });
                }}
                placeholder="0.00"
                className="w-32 rounded-lg border border-emerald-300 px-3 py-2 text-[13px] bg-white focus:border-emerald-500 outline-none"
              />
              <span className="text-[12px] text-emerald-700">për karakter</span>
            </div>
          </div>
        )}
      </div>
