...
export default function CartDrawer() {
    ...
    try {
      ...
      const data = await createOrder({
        ...
        items: items.map((it) => ({
          ...
          code: it.code,
          price: it.price,
          textPricePerCharacter: it.textPricePerCharacter,
          qty: it.qty,
  ...
  const waFullOrderText = () => {
    ...
    items.forEach((it) => {
      const customText = it.customText?.trim() || "";
      const itemUnitPrice = customText
        ? customText.length * (it.textPricePerCharacter || 0)
        : it.price;
      const itemTotal = itemUnitPrice * it.qty;
      lines.push(
        `• ${it.name}${it.code ? ` (${it.code})` : ""} x${it.qty} — ${fmtEUR(itemTotal)}${
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {it.customText && (
                      ...
                      <div className="mt-1.5 flex items-center justify-between">
                        ...
                        <span className="text-[13px] font-bold text-bronze-grad">
                          {(() => {
                             const customText = it.customText?.trim() || "";
                             const unitPrice = customText
                               ? customText.length * (it.textPricePerCharacter || 0)
                               : it.price;
                             return fmtEUR(unitPrice * it.qty);
                          })()}
                        </span>
                      </div>
                  </div>
