  ...
  return (
    ...
    >
      <div className="relative aspect-[4/4.5] overflow-hidden bg-[#cfc8bb]">
        ...
        />
        {(product.salePct ?? 0) > 0 && (
          <span className="absolute right-3 top-3 inline-flex items-center rounded-full bg-gradient-to-r from-[#d3543a] to-[#8e1f12] px-2.5 py-1 text-[10px] font-bold tracking-wide text-[#fff2e8] shadow-[0_6px_18px_rgba(142,31,18,0.45)]">
            -{product.salePct}%
          </span>
        )}
        {featuredBadge && (
      ...
      <div className="px-3.5 sm:px-4 pt-3 pb-3.5">
        ...
        {(() => {
          const displayCode = product.code || Object.values(product.colorCodes ?? {})[0] || "";
          return displayCode ? (
          ...
          {(() => {
            const base =
              product.variants.length > 1
                ...
                : product.price;
            const eff = effectivePrice(base, product.salePct);
            const sale = eff < base;
            return (
              <span className="flex items-baseline gap-1.5 min-w-0">
                {product.variants.length > 1 && (
                  <span className="text-[10px] font-semibold uppercase tracking-wide text-ivory-2/55">Nga</span>
                )}
                <span className="text-[15px] font-bold tracking-wide text-bronze-grad">{fmtEUR(eff)}</span>
                {sale && (
                  <span className="text-[11px] line-through text-ivory-2/45">{fmtEUR(base)}</span>
                )}
              </span>
            );
          })()}
