...
import { Product, CATEGORIES } from "../data/catalogue";
...
export default function ProductSheet({
  ...
  const catName = p ? CATEGORIES.find((c) => c.id === p.cat)?.name : "";
  const waText = p
    ? encodeURIComponent(`Përshëndetje Kriel! Jam i/e interesuar për produktet "${p.name}" (${p.code}), çmimi ${fmtEUR(p.price)}. Faleminderit!`)
    : "";
        ...
        <>
          ...
          >
            ...
            <div className="overflow-y-auto no-scrollbar" style={{ maxHeight: "calc(92svh - 20px)" }}>
                ...
                <img src={p.img} alt={p.name} className="w-full max-h-[46svh] object-contain object-center" />
              ...
              <div className="px-5 pt-5 pb-7">
                <p className="text-[10.5px] font-semibold tracking-[0.28em] uppercase text-bronze/80">{catName}</p>
                <h3 className="mt-1.5 font-display text-[30px] leading-tight font-semibold text-ivory">{p.name}</h3>

                <div className="mt-4 flex items-end justify-between rounded-2xl border border-line bg-ink-3/50 px-4 py-3.5">
                  <div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2.5">
                  {p.code && (
                    <Spec icon={Tag} k="Kodi i katalogut" v={p.code.replace("COD. ", "")} />
                  )}
                  {p.dims && <Spec icon={Ruler} k="Përmasat" v={p.dims} />}
                  <Spec icon={Layers} k="Materiali" v={p.mat} />
                  <Spec icon={Landmark} k="Origjina" v="Parma, Itali" />
                </div>

                <p className="mt-5 text-[13.5px] leading-relaxed text-ivory-2/80 font-light">
                  {CATEGORIES.find((c) => c.id === p.cat)?.desc} Çdo kopje numërohet dhe
                  shoqërohet me garanci autenticiteti nga fonderia.
                </p>
