...
export async function fetchProducts(options?: {
  ...
  let query = getSupabase()
    .from("products")
    .select(
      "id, category_id, name, code, price, dims, material, description, image, images, sort_order, featured, featured_order, customizable, active, discount_percent"
    )
    .order("sort_order", { ascending: true })
    .order("id", { ascending: true });
...
export async function fetchVariants(): Promise<ProductVariant[]> {
  const { data, error } = await getSupabase()
    ...
    .order("sort_order", { ascending: true })
    .order("id", { ascending: true });
  if (error) fail("Nuk u ngarkuan variantet e produkteve.", error);
