...
import { CATEGORIES, GROUP_ORDER, Product } from "../data/catalogue";
import { useCatalogAdmin } from "../admin/CatalogAdmin";
import { Reveal, Eyebrow } from "./Reveal";

// cat i produktit -> indeksi i grupit/katalogut (renditja: katalogu 1, pastaj 2, e kështu me radhë)
const CAT_GROUP_INDEX: Record<string, number> = {};
GROUP_ORDER.forEach((g, i) => g.cats.forEach((c) => (CAT_GROUP_INDEX[c] = i)));
...
export default function Catalogue({
  ...
  const { visibleCount } = useCatalogAdmin();
  ...
  const list = useMemo(() => {
    ...
    if (sort === "asc") items = [...items].sort((a, b) => a.price - b.price);
      ...
      // renditja e parazgjedhur: sipas radhës së katalogut (katalogu 1, pastaj 2, e kështu me radhë)
      items = [...items].sort(
        (a, b) => (CAT_GROUP_INDEX[a.cat] ?? 999) - (CAT_GROUP_INDEX[b.cat] ?? 999)
      );
    return items;
  }, [cat, q, sort, products]);

  // rifillo te pamja e shkurtuar sa herë ndryshon filtri/kërkimi/renditja
  useEffect(() => {
    setShowAll(false);
  }, [cat, q, sort]);
