...
import { Search, ArrowUpDown, Plus, PackageSearch, ChevronDown } from "lucide-react";
...
export default function Catalogue({
  ...
  const list = useMemo(() => {
    ...
    else if (sort === "desc") items = [...items].sort((a, b) => b.price - a.price);
    else
      items = [...items].sort(
        (a, b) => (catOrder[a.categoryId] ?? 999) - (catOrder[b.categoryId] ?? 999)
      );
    return items;
  }, [cat, q, sort, products, catOrder]);

  useEffect(() => {
    setShowAll(false);
  }, [cat, q, sort]);

  const shown = showAll ? list : list.slice(0, VISIBLE_COUNT);
        ...
        <Reveal delay={0.2}>
          <div className="sticky top-16 z-30 -mx-5 sm:mx-0 mt-10 px-5 sm:px-0 py-3 bg-ink/85 backdrop-blur-xl sm:rounded-2xl sm:border sm:border-line">
            ...
            <div className="no-scrollbar mt-3 flex gap-2 overflow-x-auto pb-0.5">
              <Chip active={cat === "all"} onClick={() => setCat("all")} label="Të gjitha" count={counts.all || 0} />
              {categories.map((c) => (
                <Chip
                  key={c.id}
                  active={cat === c.id}
        ...
        <motion.div layout className="mt-5 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
          <AnimatePresence mode="popLayout">
            {shown.map((p, i) => (
              ...
              >
                <ProductImage src={p.img} alt={p.name} className="aspect-[4/4.5]" />
                {p.featured && (
                  <span className="pointer-events-none absolute left-3 top-3 z-10 inline-flex items-center gap-1 rounded-full bg-ink/65 backdrop-blur-sm border border-bronze/40 px-2.5 py-1 text-[9px] font-bold tracking-[0.16em] uppercase text-bronze">
                    ★ E pëlqyer
                  </span>
                )}
                <span className="pointer-events-none absolute right-3 bottom-[5.8rem] z-10 w-8 h-8 rounded-full bg-bronze text-ink flex items-center justify-center opacity-0 translate-y-1.5 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-400">
        ...
        {remaining > 0 && (
          <div className="mt-8 flex justify-center">
            <button
              ...
              className="group inline-flex items-center gap-2 rounded-full border border-bronze/45 bg-ink-2/60 px-6 py-3 text-[13px] font-bold tracking-wide text-bronze transition-all duration-300 hover:border-bronze hover:bg-bronze/10"
            >
              Shfaq më shumë
              <span className="text-ivory-2/60 font-semibold">({remaining})</span>
              <ChevronDown size={15} strokeWidth={2.4} className="transition-transform duration-300 group-hover:translate-y-0.5" />
            </button>
          </div>
        )}
