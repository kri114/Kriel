...
function FileTab() {
  ...
  const [vcInput, setVcInput] = useState(String(visibleCount));
  const fileRef = useRef<HTMLInputElement>(null);
    ...
    <div className="flex flex-col gap-4">
      <div className="rounded-2xl border border-line bg-ink-2 p-5">
        <h3 className="font-display text-[20px] font-semibold text-ivory">
          Cilësimet e katalogut
        </h3>
        <p className="mt-1.5 text-[12.5px] text-ivory-2/65 font-light leading-relaxed">
          Sa produkte shfaqen fillimisht te katalogu, para se vizitori të shtypë
          <b> "Shfaq më shumë"</b>. Renditja e produkteve shtesë ndjek radhën e
          kataloguve (Gërma → Korniza → Kryqe → … ).
        </p>
        <div className="mt-3.5 flex items-center gap-2.5">
          <input
            value={vcInput}
            onChange={(e) => setVcInput(e.target.value.replace(/[^0-9]/g, ""))}
            onBlur={() => {
              const n = parseInt(vcInput, 10);
              setVisibleCount(Number.isFinite(n) && n > 0 ? n : visibleCount);
              setVcInput(String(Number.isFinite(n) && n > 0 ? n : visibleCount));
            }}
            inputMode="numeric"
            className="w-24 rounded-xl border border-bronze/40 bg-ink-3/60 px-3.5 py-3 text-[15px] font-bold text-bronze"
          />
          <span className="text-[12.5px] text-ivory-2/60">produkte fillimisht</span>
        </div>
      </div>
