...
const LINKS = [
  ...
  { href: "#te-pelqyerat", label: "Të pëlqyerat" },
          ...
          <nav className="hidden lg:flex items-center gap-7">
            {LINKS.map((l) => (
              <a
                key={l.href}
                href={l.href}
                className="text-[12px] font-medium tracking-[0.14em] uppercase text-ivory-2/80 hover:text-bronze transition-colors"
              >
                {l.label}
              </a>
            ))}
            ...
            <nav className="flex-1 flex flex-col justify-center px-8 gap-1 overflow-y-auto">
              {LINKS.map((l, i) => (
                <motion.a
                  ...
                  className="group flex items-baseline gap-4 py-3 border-b border-line/60"
                >
                  <span className="text-[11px] font-semibold tracking-[0.3em] text-bronze/70">0{i + 1}</span>
                  <span className="font-display text-[30px] font-medium text-ivory group-hover:text-bronze transition-colors">
                    {l.label}
                  </span>
                </motion.a>
              ))}
