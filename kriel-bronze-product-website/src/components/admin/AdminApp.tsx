...
export default function AdminApp() {
  ...
  const loadAll = useCallback(async () => {
    setLoadingData(true);
    try {
      const [cats, prods, vars, ords] = await Promise.all([
        fetchCategories(),
        fetchProducts(),
        fetchVariants().catch(() => [] as ProductVariant[]),
      ...
      <div className="mx-auto max-w-5xl px-5 py-6">
        ...
        {loadingData ? (
          ...
          <>
            ...
            {tab === "products" && (
              <ProductsTab categories={categories} products={products} variants={variants} reload={loadAll} />
            )}
      ...
      <div className="w-full max-w-lg rounded-3xl bg-white shadow-xl p-8 border border-slate-200">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-[12px] text-slate-500">Backend-i nuk është konfiguruar ende</p>
          </div>
        </div>
