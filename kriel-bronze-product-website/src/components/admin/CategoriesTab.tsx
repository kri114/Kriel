...
import { createCategory as apiCreateCategory, deleteCategory, updateCategory } from "@/lib/api";
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <label className="block">
            <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-slate-500">Emri</span>
            <input
              value={newDraft.name}
          ...
          <label className="block">
            <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-slate-500">Përshkrimi</span>
            <input
        ...
        <div className="mt-4">
          <ImageField folder="categories" value={newDraft.image} onChange={(v) => setNewDraft({ ...newDraft, image: v })} />
        </div>
        <button
      ...
      <div className="space-y-3">
              ...
              {!isEditing ? (
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[12.5px] text-slate-500 line-clamp-2">{c.description || "Pa përshkrim"}</p>
                    <p className="text-[11px] text-slate-400 mt-0.5">/{c.slug}</p>
                  </div>
                  <div className="flex flex-col items-end gap-2 shrink-0">
                    <div className="flex gap-1.5">
                      <button onClick={() => move(c, -1)} disabled={i === 0 || busyId === c.id} className="w-7 h-7 rounded-lg border border-slate-200 flex items-center justify-center text-slate-500 disabled:opacity-30">
                        <ArrowUp size={13} />
                      </button>
                      <button onClick={() => move(c, 1)} disabled={i === sorted.length - 1 || busyId === c.id} className="w-7 h-7 rounded-lg border border-slate-200 flex items-center justify-center text-slate-500 disabled:opacity-30">
                        <ArrowDown size={13} />
                      </button>
                    </div>
        ...
        {sorted.length === 0 && <p className="text-slate-500 text-sm">Ende nuk ka kategori.</p>}
      </div>
    </div>
  );
}
