...
import { createProduct as apiCreateProduct, deleteProduct, updateProduct } from "@/lib/api";
      ...
      {tab === "all" && (
        <>
          <div className="rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <div className="mt-4">
              <ImageField folder="products" value={newDraft.image} onChange={(v) => setNewDraft({ ...newDraft, image: v })} />
            </div>
                ...
                <div key={p.id} className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
                  {!isEditing ? (
                    ...
                    <div>
                      ...
                      <div className="mt-4">
                        <ImageField folder="products" value={draft?.image ?? ""} onChange={(v) => setDrafts((d) => ({ ...d, [p.id]: { ...d[p.id], image: v } }))} />
                      </div>
                      <label className="mt-4 flex items-center gap-2 text-[13px] text-slate-700">
                        <input
                          type="checkbox"
