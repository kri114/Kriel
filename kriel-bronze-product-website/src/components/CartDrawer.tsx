...
export default function CartDrawer() {
  ...
  const submitOrder = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Nuk u arrit lidhja me serverin. Provoni përsëri.");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          {step === "cart" && (
            <div className="p-5 flex flex-col gap-4">
              ...
              {items.map((it) => (
                <div key={`${it.productId}-${it.customText}`} className="flex gap-3 rounded-2xl border border-line bg-ink-3/40 p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {it.customText && (
                      <div className="mt-1">
                        ...
                        <input
                          value={it.customText}
                          onChange={(e) => updateCustomText(it.productId, e.target.value)}
                    ...
                    <div className="mt-1.5 flex items-center justify-between">
                      <div className="inline-flex items-center rounded-full border border-line overflow-hidden">
                        <button onClick={() => updateQty(it.productId, it.qty - 1)} className="w-6 h-6 flex items-center justify-center text-ivory-2 hover:text-bronze">
                          <Minus size={11} />
                        </button>
                  ...
                  <button onClick={() => removeItem(it.productId)} className="text-ivory-2/40 hover:text-red-400 shrink-0" aria-label="Hiq">
                    <Trash2 size={15} />
                  </button>
                </div>
              ))}
          ...
          {step === "checkout" && (
            <div className="p-5 flex flex-col gap-4">
              ...
              <div className="rounded-2xl border border-line bg-ink-3/40 p-4">
                ...
                {items.map((it) => (
                  <div key={`${it.productId}-${it.customText}`} className="flex justify-between text-[12.5px] text-ivory-2/80 py-0.5">
                    <span className="truncate pr-2">{it.name} x{it.qty}</span>
