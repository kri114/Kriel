...
export default function CartDrawer() {
  ...
  const waFullOrderText = () => {
    ...
    items.forEach((it) => {
      ...
      lines.push(
        `• ${it.name}${it.code ? ` (${it.code})` : ""}${variant ? ` — ${variant}` : ""} x${it.qty} — ${fmtEUR(it.price * it.qty)}${
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          {step === "cart" && (
            <div className="p-5 flex flex-col gap-4">
              ...
              {items.map((it) => (
                <div key={it.key} className="flex gap-3 rounded-2xl border border-line bg-ink-3/40 p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {it.customText && (
                      <div className="mt-1">
                        ...
                        <input
                          value={it.customText}
                          onChange={(e) => updateCustomText(it.key, e.target.value)}
                          className="mt-0.5 w-full rounded-lg border border-line bg-white px-2 py-1 text-[12px] text-slate-900"
                        />
                      </div>
                    )}
                    <div className="mt-1.5 flex items-center justify-between">
                      <div className="inline-flex items-center rounded-full border border-line overflow-hidden">
                        <button onClick={() => updateQty(it.key, it.qty - 1)} className="w-6 h-6 flex items-center justify-center text-ivory-2 hover:text-bronze">
                          <Minus size={11} />
                        </button>
                  ...
                  <button onClick={() => removeItem(it.key)} className="text-ivory-2/40 hover:text-red-400 shrink-0" aria-label="Hiq">
                    <Trash2 size={15} />
                  </button>
                </div>
              ))}
            ...
            <div className="p-5 flex flex-col gap-4">
              ...
              <Field label="Emri i plotë *">
                <input
                  ...
                  className="w-full rounded-xl border border-line bg-white px-4 py-3 text-[14px] text-slate-900 placeholder:text-slate-400"
                />
              </Field>
              ...
              <Field label="Adresa e dorëzimit *">
                <textarea
                  ...
                  rows={3}
                  className="w-full rounded-xl border border-line bg-white px-4 py-3 text-[14px] text-slate-900 placeholder:text-slate-400 resize-none"
                />
              </Field>
              ...
              <div className="rounded-2xl border border-line bg-ink-3/40 p-4">
                ...
                {items.map((it) => (
                  <div key={it.key} className="flex justify-between text-[12.5px] text-ivory-2/80 py-0.5">
                    <span className="truncate pr-2">
                      {it.name}
                      {(it.color || it.size) ? ` (${[it.color, it.size].filter(Boolean).join(" · ")})` : ""} x{it.qty}
                    </span>
