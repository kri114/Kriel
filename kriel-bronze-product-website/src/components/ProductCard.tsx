...
export default function ProductCard({
  ...
  const salePrice = hasDiscount ? applyDiscount(product.price, product.discountPercent) : product.price;
  ...
  return (
    ...
    >
      <div className="relative aspect-[4/4.5] overflow-hidden bg-[#cfc8bb]">
        ...
        />
        {hasDiscount && (
          <span className="absolute left-3 top-3 inline-flex items-center gap-1 rounded-full bg-red-600 px-2.5 py-1 text-[9.5px] font-bold tracking-[0.14em] uppercase text-white">
            <Tag size={10} /> -{Math.round(product.discountPercent as number)}%
          </span>
        )}
        {featuredBadge && !hasDiscount && (
        ...
        <div className="mt-1.5 flex items-center justify-between">
          {hasDiscount ? (
            <span className="flex items-baseline gap-1.5">
              <span className="text-[15px] font-bold tracking-wide text-red-400">{fmtEUR(salePrice)}</span>
              <span className="text-[11.5px] font-medium text-ivory-2/45 line-through">{fmtEUR(product.price)}</span>
            </span>
          ) : (
