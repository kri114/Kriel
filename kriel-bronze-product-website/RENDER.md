...
2. **Secrets (SMTP)**: Dashboard → **Edge Functions → Secrets** → shtoni
   këto vlera (për `infokrielshpk@gmail.com`):
   - `SMTP_HOST` = `smtp.gmail.com`
   - `SMTP_PORT` = `465`
   - `SMTP_USER` = `infokrielshpk@gmail.com`
   - `SMTP_PASS` = **App Password** 16-shkronjash, PA hapësira (krijohet te
     myaccount.google.com/apppasswords — kërkon 2-Step Verification aktive;
     fjalëkalimi normal NUK funksionon dhe jep gabimin 535)
   - `SMTP_FROM` = (lëre bosh — Gmail kërkon From == SMTP_USER)
   - `ORDER_EMAIL_TO` = (lëre bosh — parazgjedhja është
     `infokrielshpk@gmail.com` brenda kodit)
   (`SUPABASE_URL` dhe `SUPABASE_SERVICE_ROLE_KEY` jepen automatikisht.)
