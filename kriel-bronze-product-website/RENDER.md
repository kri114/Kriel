...
## 10. Ngjyra blu në fushat e kërkimit/renditjes/formularëve — rregulluar

Shfletuesit (Chrome/Edge/Safari) shpesh vendosin një sfond blu të zbehtë mbi
fushat kur: (1) OS-i është në "dark mode" dhe faqja nuk deklaron
`color-scheme`, ose (2) shfletuesi "autofill"-on një fushë (emër, telefon,
adresë, kërkim) dhe injoron ngjyrën tuaj të sfondit. Kjo shfaqej te "Kërko
produkt", "Renditja", dhe fushat e formularit të porosisë/panelit.

**Zgjidhur në `src/app/globals.css`:**
- `color-scheme: light` në `<html>` — detyron shfletuesin të përdorë stil të
  çelët për elementët nativë (dropdown-e, checkbox-e), pavarësisht nga
  cilësimi i errët i OS-it të vizitorit.
- Override i plotë i `-webkit-autofill` — sfondi kthehet gjithmonë i bardhë
  me tekst të errët të lexueshëm, në vend të blusë të zbehtë të shfletuesit.
- Të gjitha fushat e panelit (input/select/textarea) tani kanë `bg-white`
  shprehimisht, si siguri shtesë.
