...
create table if not exists products (
  ...
  discount_percent numeric(5, 2), -- "Ulje" % (0-100); null/0 = no discount, not on offer
  created_at     timestamp        not null default now()
);

-- Safe to re-run on a database created before the "multiple photos" feature.
alter table products add column if not exists images jsonb not null default '[]'::jsonb;
-- Safe to re-run on a database created before the "Ulje"/sale feature.
alter table products add column if not exists discount_percent numeric(5, 2);

-- Color/size options for a product. Each row is one selectable combination
-- (e.g. color="E artë", size="M") with its own optional price override and
-- optional photo. A product with no rows here behaves exactly like before
-- (single price, single photo, no selectors on the public site).
create table if not exists product_variants (
  id             serial primary key,
  product_id     integer        not null references products (id) on delete cascade,
  color          varchar(100)   not null default '',
  size           varchar(100)   not null default '',
  price          numeric(10, 2),  -- null = inherit the product's base price
  image          text           not null default '',
  sort_order     integer        not null default 0,
  created_at     timestamp      not null default now()
);
...
create policy "admin full variants"
