...
function buildText(o: OrderRow, items: OrderItem[], total: number): string {
  ...
  for (const it of items) {
    ...
    if (it.size) {
      lines.push(`     ↳ Madhësia e zgjedhur: "${it.size}"`);
    }
    if (it.color) {
      lines.push(`     ↳ Ngjyra e zgjedhur: "${it.color}"`);
    }
    if (it.customText) {
...
function buildHtml(o: OrderRow, items: OrderItem[], total: number): string {
  const rows = items
    .map(
      (it) => `
      <tr>
        <td style="padding:8px 10px;border-bottom:1px solid #eee;">${escapeHtml(it.name)}${it.code ? ` <span style="color:#999;font-size:12px;">(${escapeHtml(it.code)})</span>` : ""}${it.size ? `<br/><span style="color:#a97e42;font-size:12px;">Madhësia: “${escapeHtml(it.size)}”</span>` : ""}${it.color ? `<br/><span style="color:#a97e42;font-size:12px;">Ngjyra: “${escapeHtml(it.color)}”</span>` : ""}${it.customText ? `<br/><span style="color:#a97e42;font-size:12px;">Tekst i porositur: “${escapeHtml(it.customText)}”</span>` : ""}</td>
